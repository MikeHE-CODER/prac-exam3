...
#include <string.h>
using namespace std;
Helicopter::Helicopter(int w, string n) :AirCraft(w),name(n){}    // creates a Helicopter with weight w and name n
string Helicopter::get_name(){return name;};     
void Helicopter::set_name(string a){this->name=a;};            
void Helicopter::fly(int headwind, int minutes){
    float chushi=get_fuel;
    if (headwind<40){
        this->set_fuel(get_fuel-=0.002*minutes);
    }else{
        this->set_fuel(get_fuel-=0.004*minutes);
    }
